...
// handleGatewayAuthMitigation handles the gateway authentication risk mitigation
func handleGatewayAuthMitigation(req map[string]interface{}) *C.char {
	...
	mode, _ := formData["auth_mode"].(string)
	token, _ := formData["token_value"].(string)

	// Generate random token if requested
	...
	if mode == "token" {
		if err := core.UpdateJSONConfig(configPath, "gateway.auth.enabled", true); err != nil {
			return C.CString(fmt.Sprintf(`{"success": false, "error": "failed to enable auth: %v"}`, err))
		}
		if err := core.UpdateJSONConfig(configPath, "gateway.auth.mode", "token"); err != nil {
			return C.CString(fmt.Sprintf(`{"success": false, "error": "failed to set mode: %v"}`, err))
		}
		if err := core.UpdateJSONConfig(configPath, "gateway.auth.token", token); err != nil {
			return C.CString(fmt.Sprintf(`{"success": false, "error": "failed to set token: %v"}`, err))
		}
		core.UpdateJSONConfig(configPath, "gateway.auth.password", "")

		return C.CString(fmt.Sprintf(`{"success": true, "message": "Token set to: %s"}`, token))
	}

	return C.CString(`{"success": false, "error": "only token mode supported now"}`)
}
...
func handleGatewayBindMitigation(req map[string]interface{}) *C.char {
	...
	if err := core.UpdateJSONConfig(configPath, "gateway.bind", bindAddr); err != nil {
		return C.CString(fmt.Sprintf(`{"success": false, "error": "%v"}`, err))
	}

	return C.CString(fmt.Sprintf(`{"success": true, "message": "Bind address set to %s"}`, bindAddr))
}
...
func handleSandboxDisabledDefaultMitigation(req map[string]interface{}) *C.char {
	...
	if err := core.UpdateJSONConfig(configPath, "agents.defaults.sandbox.mode", sandboxMode); err != nil {
		return C.CString(fmt.Sprintf(`{"success": false, "error": "%v"}`, err))
	}

	return C.CString(fmt.Sprintf(`{"success": true, "message": "Default sandbox mode set to %s"}`, sandboxMode))
}
